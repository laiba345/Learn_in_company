...
<template>
  <v-layout row wrap>
          ...
          <v-container fluid style="padding-top: 10px;">
            <v-layout row>
              ...
              <div v-if="checktab === 2" style="display: flex; justify-content: space-between; margin-bottom: 16px;">
                ...
                <v-btn class="btn_content" outline color="#333" style="height: 36px;"
                  @click.exact="resetSFCInformation"><v-icon style="font-size: 18px; margin-right: 2px;">refresh</v-icon>Reset</v-btn>
                <v-btn v-if="isengineer && isadmin" color="primary" class="btn_content" @click.exact="addTransit">Sync</v-btn>
              </div>
            </v-layout>
            <!-- 表格 -->
            <v-layout row wrap>
    ...
    <add-sfc-transmission ref="addTransmission" :bulist="buItems" :currenTtime="nowDateTime" @transitSFC="uploadSFCData"></add-sfc-transmission>
    <!-- <v-test></v-test> -->
  </v-layout>
</template>
  ...
  methods: {
    ...
    async getCurrentTimes() {
      ...
      if (res.status) {
        ...
        this.endDate = this.$route.query.endDate || this.currenTtime[0];
        this.startDate = this.$route.query.startDate || moment(this.endDate).subtract(6, 'month').format('YYYY-MM-DD');
    ...
    async gettestdataList() {
      ...
      console.log('排序后的testdata:', this.testdata);
      this.testdataLoading = false;
    },
    async getParentChildrens() {
      console.log('getParentChildrens called with:', this.parentsernum, this.parentStartDate, this.parentEndDate, this.option);
