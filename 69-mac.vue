      ...
      <v-widget title="MAC Search">
        <div slot="widget-content">
          <v-flex>
            ...
            <!-- 在App.vue中添加 -->
            <!-- <div class="message-preview" :style="{top: alert.visible ? '20px' : '-100px', opacity: alert.visible ? 1 : 0}"> -->
              <v-alert
              ...
              >
                <div class="alert-content">
                  <v-icon class="alert-icon">{{ alert.icon }}</v-icon>
                  <div class="alert-message">{{ alert.message }}</div>
                </div>
                <v-icon class="alert-close" @click="alert.visible = false">mdi-close</v-icon>
              </v-alert>
            <!-- </div> -->
          </v-flex>
...
export default {
  ...
  methods: {
    ...
    async getMacSearchData() {
      ...
      if (!newMac) {
        this.showAlert('red', 'Please enter search content');
        return; 
      }
      const serviceConfig = {
        'MAC': {
      ...
      if (!res.status) {
        this.showAlert('red', res.payload.message);
        return;
      }
      this[config.resultKey] = res.payload.data;
      // this.showAlert('success', `${res.payload.message}, ${res.payload.time}`);
      this.showSuccess(`${res.payload.message}`);
    },
    ...
    showMessage(options) {
      ...
      Object.assign(this.alert, config);
    ...
    showSuccess(message, duration = 3000) {
      this.showMessage({ message, type: 'success', duration });
    },
    
    showError(message, duration = 3000) {
      this.showMessage({ message, type: 'error', duration });
    },
    
    showWarning(message, duration = 3000) {
      this.showMessage({ message, type: 'warning', duration });
    },
    
    showInfo(message, duration = 3000) {
      this.showMessage({ message, type: 'info', duration });
    }
  }
}; 
</script>
