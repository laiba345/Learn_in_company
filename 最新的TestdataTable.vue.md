      ...
      <v-layout row wrap>   
        <v-flex xl12 sm12 md12 pa-0>
          <v-data-table
            ...
            @update:pagination="changeTablePage"
          >
            <!-- 表单内容 -->
          ...
          <div v-if="testDataSource.length > 0">
            <v-btn flat color="indigo" style="border-radius: 4px;" @click="exportExcel">
...
import { getTestRecordList, getTestRecordDetail, downloadTestRecords } from '../../api/basic.js';
...
export default {
  ...
  computed: {
    ...
    noDataText() {
      return this.searchClick ? 'No matching records found' : 'No data available';
    },
    sernumCount() {
  ...
  methods: {
    ...
    async getCurrentTimes() {
      ...
      if (res.status) {
        ...
        if (this.sernum) this.handleSearchClick();
      }
    },
    changeTablePage(val) {
      // 切换表格每页显示条数
    ...
    async toggleRowExpand(props) {
      ...
      try {
        ...
        const { data: res } = await getTestRecordDetail(params);
        if (res.status) {
    ...
    async exportExcel() {
      ...
      try {
        ...
        const pollDownloadData = async () => {
          ...
          try {
            const { data: downloadRes } = await downloadTestRecords({ ...params });
