...
// const open = (type, editInfo) => {
//   console.log('type && editInfo', type, editInfo);
//   dialogType.value = type;
//   dialog.value = true;
//   // if (type === 'EDIT') param.value = editInfo;
//   if (type === 'EDIT') {
//     param.value = editInfo;
//     param.value.ccData = editInfo.cc ? [...editInfo.cc] : [];
//     handleCc(); 
//   } else if (type === 'COPY') {
//     // 复制模式：拷贝数据，但要把唯一标识 row_id 删掉！
//     param.value = editInfo;
...
// 🛠️ 【修改点 3】: 重写 open 函数，在加载 COPY 数据前提前拉取列表并上锁
const open = async (type, editInfo) => {
  ...
  if (type === 'EDIT') {
    ...
    // 1. 开启锁：通知侦听器千万不要清空 group_name
    isInitializing.value = true; 

    // 2. 提前拉取目标 Site 的群组列表，防止下拉框因为没有候选数据而显示空白
    if (editInfo.site) {
      try {
...
const submitForm = (formRef) => {
  formRef.validate(async (valid) => {
    ...
    if (dialogType.value === 'ADD' || dialogType.value === 'COPY') {
      console.log('Add param.value', param.value);
      ...
      console.log('Edit param.value', param.value);
...
const getTableGroupList = async () => {
  ...
  console.log('res', res);
  if (!res.status) return publicWayStore().showElMessage(res.message);
  groupList.value = res.payload.data.filter((item) => {
    if (item) return true;
    if (!item) return false;
  });
