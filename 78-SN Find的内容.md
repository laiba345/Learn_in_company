    ...
    <v-card flat>
      <v-layout row wrap>   
        <v-flex xl12 sm12 md12 pa-0>
          ...
          <json-to-excel :jsonData="downTestRecord" :disable-export="true" excelName="Test Record" @export-click="handleExportClick"></json-to-excel>
        </v-flex>            
      </v-layout>
    </v-card>
...
export default {
  ...
  computed: {
    ...
    downTestRecord() {
      if (this.testDataSource.length.length < 1) return [];
      const keyMap = {
        recttime: 'Record Time',
        tottime: 'Test Time',
        testtime: 'Log Time'
      };
      return this.testDataSource.map(obj => {
        const newObj = {};
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (routeSernum) {
        this.$emit('update:sernum', routeSernum); // 同步到父组件
        this.$nextTick(() => {
  ...
  mounted() {
    setTimeout(() => {
      this.pagination.totalItems = this.testDataSource.length;
    }, 1000);
    this.getCurrentTimes(); 
  },
  methods: {
    ...
    async searchDataSummary() {
      ...
      try {
        ...
        const workId = match ? match[1] : rawUsername;
        ...
        const pollData = async () => {
          ...
          try {
            ...
            if (status === 'ADDED' || status === 'PENDING') {
              ...
              this.yeildLoading = false; // 结束 loading 状态
            } 
      ...
      <v-widget title="SN Find">
        <div slot="widget-content">
          <v-container fluid style="padding-top: 10px;">
            ...
            <!-- 表格 -->
            <v-layout row wrap>
              <v-flex sm12 v-if="checktab === 0" style="margin-top: 24px;">
                <testdata-table :sync-sernum="parentSernum" :auto-search="!hasAutoSearched" @update:sernum="val => parentSernum = val" @searched="onSubComponentSearched"/>
              </v-flex>
              <v-flex sm12 v-if="checktab === 1" style="margin-top: 24px;">
                <parentdata-table />
              </v-flex>
              <v-flex sm12 v-if="checktab === 2" style="margin-top: 24px;">
                <sfc-transmission-info />
              </v-flex>
            </v-layout>
          </v-container>
        </div>
...
export default {
  ...
  created () {
    ...
    if (this.$route.query.sernum) {
      this.parentSernum = this.$route.query.sernum;
    }
  }, 
  methods: {
    // 刷新维持页签
    handleChecktab() {
      const adminCookie = this.$cookies.get('admin');
      // 当 cookie 存在且是字符串时，才尝试用 Buffer 解码
      if (adminCookie && typeof adminCookie === 'string') {
        try {
          this.isadmin = Buffer.from(adminCookie, 'base64').toString('utf-8') === 'true';
        } catch (e) {
          console.error('解析 admin cookie 失败:', e);
          this.isadmin = false;
        }
      } else {
        // 如果没有 cookie，或者 cookie 直接就是布尔值 true (无需解码)
        this.isadmin = adminCookie === 'true' || adminCookie === true;
      }
      // 处理 session 路由恢复逻辑
      let path = JSON.parse(sessionStorage.getItem('route'));
      if (path && path.from === '/') {
        this.checktab = path.checktab || 0;
      }
    },
    onSubComponentSearched() {
      this.hasAutoSearched = true;
    },
    // 获取服务器信息
