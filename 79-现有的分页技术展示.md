        ...
        <v-container fluid style="padding: 0">
          <v-form ref="form" lazy-validation>
            <v-layout row wrap>
              <v-layout row wrap class="layout-margin">
                <div class="search-input">
                  <v-text-field class="hidden-sm-and-down MacManageSearch" solo style="width: 200px;" placeholder="Serial Number"
                    v-model="sernum" v-on:keyup.enter="searchDataSummary" color="primary" clearable>
                    <template slot="append">
                      <button id="searchButton">
                        <i class="iconfont icon-search" style="font-size: 18px;" @click="searchDataSummary"></i>
                      </button>
                    </template>
                  </v-text-field> 
                </div>
              ...
              <v-layout row wrap class="layout-margin">
                <v-select v-model="option" outline color="primary" class="SelectBlockOffset" dense style="width: 160px; margin-right: 10px;" :items="parentItems"
                  placeholder="BU" :menu-props="{ bottom: true, offsetY: true }" @change="changeOption"></v-select>
              </v-layout>
              ...
              <v-layout row wrap class="layout-margin">
                ...
                <!-- 开始时间区域 -->
                <div style="margin-right: 10px" class="search-input">
                  <date-picker :endDate="endDate" :confirmValue="startDate" @saveDate="saveStartDate"></date-picker>
                </div>
                <!-- 结束时间区域 -->
                <div class="search-input">
              ...
              <v-layout row wrap class="layout-margin">
                <div :style="{ width: '105px', height: '36px', padding: '0 0 0 7px', 'margin-left': '-28px' }">
                  <v-btn class="btn_content" color="primary" :loading="yeildLoading" @click="searchDataSummary" :disabled="yeildLoading">
                    Search
                    <span slot="loader">Loading...</span> 
                  </v-btn>
                </div>
          ...
          >
            ...
            <template slot="items" slot-scope="props">
              <tr @click="props.expanded = !props.expanded">
                ...
                <td>
                  <a v-if="option === 'Get Parent'" @click="clickSerialNumber(props.item.parent_serial_number)">{{ props.item.parent_serial_number }}</a>
                  <span v-else>{{ props.item.parent_serial_number }}</span>
                </td>              
                <td>{{ props.item.parent_uut_type }}</td>
                <td>
                  <a v-if="option === 'Get Child'" @click="clickSerialNumber(props.item.child_serial_number)">{{ props.item.child_serial_number }}</a>
                  <span v-else>{{ props.item.child_serial_number }}</span>
                </td>
            ...
            <template slot="actions-append">
              <div style="display: flex;align-items: center;">
                ...
                <span class="mx-1">Go to</span>
                <v-select v-model="pagination.page" outline dense class="SelectBlockOffset" :items="pagesCount" style="width:80px;margin-right:10px;" :menu-props="{ bottom: true, offsetY: true }"></v-select>
              </div>
            </template>
          </v-data-table>
          <json-to-excel :jsonData="genealogyInfo" excelName="Parent/Children"></json-to-excel>
        </v-flex>            
      </v-layout>
    </v-card>
  </div>
...
import store from '../../../vision/store';
import { getCurrentTime } from '../../../genius/api/getCurrentTime.js';
import { getParentChildren } from '../../api/getParentChildren.js';
import moment from 'moment';
...
export default {
  ...
  watch: {
    genealogyInfo: {
      handler() {
        this.pagination.totalItems = this.genealogyInfo.length;
        // eslint-disable-next-line no-unused-expressions
        this.genealogyInfo.length > 15 ? this.hidePage = false : this.hidePage = true;
      },
      immediate: true,
      deep: true,
    }
  ...
  methods: {
    ...
    saveStartDate(date) {
      this.startDate = date;
      if (this.sernum) this.searchDataSummary();
    },
    saveEndDate(date) {
      this.endDate = date;
      if (this.sernum) this.searchDataSummary();
    ...
    async searchDataSummary() {
      // 校验是否输入了序列号
      if (!this.sernum.trim()) {
        return this.$message({ message: 'Please input Serial Number', type: 'warning' });
      }
      this.yeildLoading = true;
      this.isEmptySearch = true;
      try {
        const { data: res } = await getParentChildren({ 
          serial_number: this.sernum,  
        ...
        if (!res.status) { 
          ...
          return this.$message({
            showClose: true,
            message: res.msg || 'Fetch data failed',
            type: 'error', 
            duration: 3000
          });
        }
        // 赋值给本组件的 genealogyInfo 并排序
        this.genealogyInfo = (res.payload.data || []).sort((a, b) => {
          return new Date(b.updated_at) - new Date(a.updated_at); 
        });
      } catch (error) {
        console.error(error);
        this.$message({ message: 'Request Error', type: 'error' });
      } finally {
        this.yeildLoading = false;
      }
    }
        ...
        <v-container fluid style="padding: 0">
          <v-form ref="form" lazy-validation>
            <v-layout row wrap>
              ...
              <!-- 搜索时间段 -->
              <v-layout row wrap class="layout-margin">
                <div class="status-select status-label-time FontH8">
                  Time:
                </div>
                <!-- 开始时间区域 -->
                <div class="search-input">
                ...
                <!-- 结束时间区域 -->
                <div style="margin-right: 20px" class="search-input">
                  <date-picker :startDate="startDate" :lastDate="currenTtime.split('T')[0]" :confirmValue="endDate" @saveDate="saveEndDate"></date-picker>
                </div>
              </v-layout>

              <v-layout row wrap class="layout-margin">
                <!-- 勾选搜索的筛选框 -->
                <div class="status-select status-label FontH8">
                  Result:
                </div>
                <div class="status-select mr-10">
                  <v-checkbox v-model="result" value="S" :label="`Start `" color="#9d4edc"></v-checkbox>
                </div>
                <div class="status-select mr-10">
                  <v-checkbox v-model="result" value="P" :label="`Pass `" color="#9d4edc"></v-checkbox>
                </div>
                <div class="status-select mr-10">
                  <v-checkbox v-model="result" value="F" :label="`Fail `" color="#9d4edc"></v-checkbox>
                </div>
                <div class="status-select mr-20">
                  <v-checkbox v-model="result" value="A" :label="`Abort `" color="#9d4edc"></v-checkbox>
                </div>
              </v-layout>
              <v-layout row wrap class="layout-margin">
                <div class="status-select status-label FontH8">
                ...
                <div :style="{ width: '105px', height: '36px', padding: '0 0 0 7px', 'margin-left': '10px' }">
                  <v-btn class="btn_content" color="primary" :loading="recordLoading" @click="handleSearch" :disabled="recordLoading">
                    Search
                    <span slot="loader">Loading...</span> 
                  </v-btn>
                </div>
    ...
    <v-card flat>
      <v-layout row wrap>   
        <v-flex xl12 sm12 md12 pa-0>
          <v-data-table
            :headers="headers"
            :items="Array.isArray(recordData.data) ? recordData.data : []"
            :pagination.sync="pagination"
            :rows-per-page-items="[15, 50, 100, 200, 500, 1000]"
            :total-items="recordData.data_sum"
            class="elevation-1 BlockOffsetTable ThemeTable TableNumber"
            :hide-actions="recordData.data_sum <= 15"
          ...
          >
            ...
            <template slot="actions-append">
              <div style="display: flex;align-items: center;">
                <span class="mx-4">{{ recordData.start_index }}-{{ recordData.end_index }} of {{ recordData.data_sum }}</span>
                <v-pagination v-model="page" :length="recordData.page_sum" :total-visible="5"></v-pagination>
                <span class="mx-1">Go to</span>
                <v-select v-model="page" outline dense class="SelectBlockOffset" :items="pagesCount" 
                  style="width:80px; margin-right:10px;" :menu-props="{ bottom: true, offsetY: true }"></v-select>
              </div>
            </template>
          </v-data-table>
          <div v-if="recordData.data_sum > 0">
      ...
      <v-dialog v-model="dialog" persistent width="300">
        <v-card color="primary" dark>
          <v-card-text>
            <div style="display: flex; justify-content: space-between;">
              <span>Loading data, please wait...</span>
              <v-icon v-if="recordLoading || downloadLoading" style="font-size: 18px; cursor: pointer" color="#FFFFFF" @click="closeLoadingData">close</v-icon>
            </div>
            <v-progress-linear indeterminate color="white" class="mb-0"></v-progress-linear>
          </v-card-text>
        </v-card>
...
export default {
  ...
  computed: {
    // 动态生成 Go to 选项数组：[1, 2, 3, ..., totalPages]
    pagesCount() {
      return Array.from({ length: this.recordData.page_sum }, (_, index) => index + 1);
    },
    noDataText() {
      return Object.keys(this.queryParams).length ? 'No matching records found' : 'No data available';
    },
    sernumCount() {
      if (!this.sernum.trim()) return 'Serial Number - 0';
      return 'Serial Number - ' + this.sernum.trim().split(this.re).length;
    ...
    dialog() {
      return this.recordLoading || this.downloadLoading;
    },
    formattedResult() {
  ...
  methods: {
    handleTime() {
      if (!this.currenTtime || !this.currenTtime.length || this.endDate) return;
      this.endDate = this.currenTtime.split('T')[0];
      this.startDate = moment(this.endDate).subtract(6, 'month').format('YYYY-MM-DD');
    },
    async getTestLog(data) {
      const time_stamp = data.testtime;
    ...
    async searchDataSummary() {
      const res = await this.getCurrentTime();
      if (!res.status) {
        return this.$message({
          showClose: true,
          message: res.payload.message || 'Service Error, please contact Genius Team',
          type: 'error',
          duration: 3000
        });
      }
      ...
      const pollData = async() => {
        ...
        try {
          const { data: res } = await getTestRecordList({ ...params });
          if (!res.status) {
            this.$message({
              showClose: true,
              message: res.msg || 'Service Error, please contact Genius Team',
              type: 'error',
              duration: 3000
            });
            this.recordLoading = false;
          ...
          const isPolling = status === 'ADDED' || status === 'PENDING';
          if (isPolling) {
            this.pollingTimer = setTimeout(pollData, 2000);
          } else if (status === 'SUCCESS') {
            this.recordData = res.payload.data.result || {};
          } else if (status === 'FAILURE') {
            this.$message({
              showClose: true,
              message: 'Service Error, please contact Genius Team',
          ...
          if (!isPolling) this.recordLoading = false;
        } catch (error) {
          this.recordLoading = false;
          return this.$message({
            showClose: true,
    ...
    async exportExcel() {
      ...
      const pollDownloadData = async () => {
        try {
          const { data: res } = await downloadTestRecords({ ...params });
          if (!res.status) {
            this.$message({
              showClose: true,
              message: res.msg || 'Service Error, please contact Genius Team',
          ...
          if (status === 'ADDED' || status === 'PENDING' || status === 'RECEIVED') {
            ...
            this.downloadLoading = false;
            const url = res.payload.data.result.url;
            if (!url) {
              this.$message({
                showClose: true,
                message: res.payload.data.result.msg,
                type: 'error', 
                duration: 3000, 
              });
              return;
